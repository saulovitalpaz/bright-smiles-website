...
type Form = { name: string; batch: string; reconstitutedAt: string; procedureType: FacialProcedureType; quantity: string; concentration: string; price: string; active: boolean };
type Movement = { id: number; quantity: number; reason: string; createdAt: string };
const blank: Form = { name: "", batch: "", reconstitutedAt: "", procedureType: "botulinum-toxin", quantity: "", concentration: "", price: "", active: true };
...
export default function AdminStock() {
  ...
  const openProduct = (product: StockProduct | null) => {
    ...
    setForm(product ? { name: product.name, batch: product.batch || "", reconstitutedAt: product.reconstitutedAt?.slice(0, 10) || "", procedureType: product.procedureType, quantity: String(product.quantity), concentration: product.concentration === null ? "" : String(product.concentration), price: String(product.price), active: product.active } : blank);
  };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const product = { name: form.name.trim(), batch: form.batch.trim() || null, reconstitutedAt: form.reconstitutedAt || null, procedureType: form.procedureType, stockUnit: thread ? "unit" : "ml", concentration: thread ? null : concentration, price, active: form.active };
    setBusy(true);
    try {
  ...
  const visible = products.filter(p => `${p.name} ${p.batch || ""}`.toLocaleLowerCase("pt-BR").includes(search.trim().toLocaleLowerCase("pt-BR")) && (!filter || p.procedureType === filter));
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <Dialog open={!!modal} onOpenChange={open => { if (!open && !busy) setModal(null); }}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg"><DialogHeader><DialogTitle>{modal === "product" ? selected ? "Editar produto" : "Novo produto" : modal === "adjust" ? "Ajustar estoque" : "Histórico do estoque"}</DialogTitle><DialogDescription>{modal === "product" ? "Saldo em ml para líquidos e unidades para fios. Preço por ml ou unidade." : selected?.name}</DialogDescription></DialogHeader>
        ...
        {modal === "product" && <form onSubmit={submit} className="space-y-4"><label className={labelClass}>Nome do produto<Input required maxLength={160} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></label><label className={labelClass}>Classe<select className={selectClass} disabled={!!selected} value={form.procedureType} onChange={e => setForm({ ...form, procedureType: e.target.value as FacialProcedureType, concentration: "" })}>{stockClasses.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}</select></label>
          <div className="grid gap-4 sm:grid-cols-2"><label className={labelClass}>Lote<Input maxLength={100} value={form.batch} onChange={e => setForm({ ...form, batch: e.target.value })} /></label><label className={labelClass}>Data de reconstituição<Input type="date" value={form.reconstitutedAt} onChange={e => setForm({ ...form, reconstitutedAt: e.target.value })} /></label></div>
          <p className="text-xs text-muted-foreground">Use um cadastro por lote e reconstituição para distinguir as apresentações nas aplicações. Deixe a data em branco quando não se aplicar.</p>
