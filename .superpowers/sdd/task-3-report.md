...
## Review fix 2: HTTP schedule validation coverage

- Added a real request-level regression test that mounts the production `PUT /leads/:id` handler on an in-memory Express app, sends invalid JSON payload data, and asserts `400` plus `{ "error": "Invalid scheduled date" }`.
- Verified the invalid request is rejected before Prisma by asserting the fake `prisma.lead.update()` is never called.
- Added an explicit `YYYY-MM-DD` normalization assertion in the runtime schedule helper coverage.
- Extracted the production lead update logic into `server/routes/leads.js` and reused that handler from `server/index.js` so the test exercises the real route behavior without a live database.
- Preserved the existing `DATABASE_URL` limitation noted above; this fix does not require a live DB.

### Exact verification output for review fix 2
