...
export default function VPVetApp() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center p-4 lg:p-10 font-sans text-slate-900 printable-area">
      
      {/* Container to restrict max width for better ergonomics and layout */}
        ...
        <header className="flex justify-between items-center bg-white p-4 px-8 rounded-3xl shadow-soft no-print">
           <div className="flex items-center gap-6">
              <div className="w-16 h-16 relative flex-shrink-0">
                 ...
                 <img src="/assets/logo.png" alt="VPVet Logo" className="w-full h-full object-contain" onError={(e) => { (e.target as any).src = "https://vitalpaz.vet/wp-content/uploads/2023/07/cropped-Favicon-32x32.png" }} />
              </div>
              <div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
           ...
           <div className="xl:col-span-4 space-y-6">
              <section className={cn(
                 "bg-white p-6 rounded-[32px] shadow-lg h-full min-h-[500px] flex flex-col transition-all relative overflow-hidden ring-4 ring-transparent print:ring-0",
                 results.length > 0 ? "ring-emerald-50" : "opacity-90"
              )}>
                 <div className="flex items-center gap-3 border-b-2 border-slate-50 pb-6 mb-6">
                 ...
                 {results.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-slate-100 gap-6 py-20">
                       <SearchCheck className="w-16 h-16 opacity-10" />
                       <p className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-300">Nenhum resultado gerado</p>
                    </div>
                 ) : (
                    <div className="space-y-4 flex-1">
                       <div className="print-header hidden print:block mb-8 text-center border-b pb-6">
                          <h1 className="text-2xl font-black text-slate-800">VPVET - DIAGNÓSTICO POR IMAGEM</h1>
                          <p className="text-xs font-bold text-slate-500">RELATÓRIO DE REFERÊNCIAS ULTRASSONOGRÁFICAS</p>
                          <div className="grid grid-cols-2 gap-4 mt-6 text-left text-[10px] px-4">
                             <div><span className="font-black">Espécie:</span> {patient.species === 'canine' ? 'Canino' : 'Felino'}</div>
                             <div><span className="font-black">Sexo:</span> {patient.gender === 'male' ? 'Macho' : 'Fêmea'}</div>
                             <div><span className="font-black">Peso:</span> {patient.weight} kg</div>
                             <div><span className="font-black">Idade:</span> {patient.age} anos</div>
                          </div>
                       </div>

                       <div className="space-y-4">
                        {results.map((org, i) => (
                            <div key={i} className="space-y-3 animate-in slide-in-from-right duration-500">
                              <div className="flex items-center gap-2 text-emerald-600 font-black text-[9px] uppercase tracking-widest bg-emerald-50/50 p-2 px-3 rounded-xl w-fit">
                                  {org.icon} <span>{org.name}</span>
                              </div>
                              {org.items.map((item: any, j: number) => (
                                  <div key={j} className={cn(
                                    "p-4 rounded-[24px] border transition-all space-y-3",
                                    item.interpretation.status === 'danger' ? "bg-rose-50/50 border-rose-100" : item.interpretation.status === 'warning' ? "bg-amber-50/50 border-amber-100" : "bg-emerald-50/30 border-emerald-50"
                                  )}>
                                    <div className="flex justify-between items-center">
                                        <h5 className="text-[9px] font-black text-slate-400 uppercase tracking-wide truncate max-w-[150px]">{item.label}</h5>
                                        <div className={cn(
                                    ...
                                    <div className="flex items-baseline gap-1.5">
                                        <span className={cn("text-xl font-black", item.interpretation.status === 'danger' ? "text-rose-900" : item.interpretation.status === 'warning' ? "text-amber-900" : "text-emerald-900")}>{item.value}</span>
                                        <span className="text-[9px] font-black text-slate-400 uppercase">{item.unit}</span>
                                    </div>
                                    <div className="p-2.5 bg-white/80 rounded-xl text-[8px] font-bold text-slate-400 flex justify-between items-center mb-1">
                                        <span className="tracking-widest uppercase opacity-60">REF:</span>
                                        <span className="text-slate-600 font-black truncate max-w-[150px]">{Array.isArray(item.reference) ? `${item.reference[0]} - ${item.reference[1]}` : item.reference === null ? 'N/A' : item.reference} {item.reference !== null ? item.unit : ''}</span>
                                    </div>
                                    
                                    {/* Clinical Suspicion Alert */}
                                    {item.interpretation.suspicion && (
                                       <div className={cn(
                                          "px-3 py-2 rounded-xl text-[9px] font-bold mt-2",
                                          item.interpretation.status === 'danger' ? "bg-rose-100/50 text-rose-800" : "bg-amber-100/50 text-amber-800"
                                       )}>
      ...
      <style jsx global>{`
        @media print {
          ...
          body, .printable-area { background: white !important; padding: 0 !important; }
          .printable-area > div { max-width: 100% !important; margin: 0 !important; }
          .shadow-soft, .shadow-lg, .shadow-xl { box-shadow: none !important; }
          .rounded-3xl, .rounded-[32px], .rounded-[48px], .rounded-[24px] { border-radius: 8px !important; border: 1px solid #eee !important; }
          .grid { display: block !important; }
          .xl\\:col-span-12, .xl\\:col-span-8, .xl\\:col-span-4 { width: 100% !important; margin-bottom: 2rem; }
